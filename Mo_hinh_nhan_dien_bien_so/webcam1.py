...
yolo_LP_detect = torch.hub.load('cam_vao', 'custom', path='model/license_plate.pt', force_reload=True, source='local')
yolo_license_plate = torch.hub.load('cam_vao', 'custom', path='model/letters.pt', force_reload=True, source='local')
        ...
        for cc in range(0, 2):
            for ct in range(0, 2):
                ...
                if lp != "unknown":
                    ...
                    files = {'image': ('img1.jpg', image_data, 'image/jpeg')}
            
                    response = requests.post(BASE + "camvao", data=payload, files=files)
                    response_data = response.json()
                    
                    if response_data is not None and 'ket_qua' in response_data:
                        kq = response_data['ket_qua']
                        if kq == 'xe_da_co_trong_bai':
                            print('Xe nay dang o trong bai')
                        if kq == 'xe_chua_o_trong_bai':
                            # Mở rào chắn lên
